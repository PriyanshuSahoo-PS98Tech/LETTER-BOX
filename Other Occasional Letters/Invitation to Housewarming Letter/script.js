document.addEventListener('DOMContentLoaded', function () {
    ...
    const housewarmingDocument = document.getElementById('housewarmingDocument');
    const blessingContainer = document.getElementById('blessingContainer');

    let isOpened = false;

    function openHousewarmingLetter() {
        if (isOpened) return;
        isOpened = true;
        ...
        setTimeout(function () {
            housewarmingDocument.classList.add('show');
    ...
    function createBlessingEffects() {
        ...
        for (let i = 0; i < 70; i++) {
            ...
            blessing.style.animationDuration = (3.2 + Math.random() * 2.7) + 's';
    ...
    envelope.addEventListener('click', openHousewarmingLetter);

    envelope.addEventListener('keydown', function (event) {
        if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            openHousewarmingLetter();
        }
    });

    window.addEventListener('keydown', function (event) {
        if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'p') {
            if (housewarmingDocument.classList.contains('show')) {
                setTimeout(function () {
                    window.print();
                }, 100);
            }
